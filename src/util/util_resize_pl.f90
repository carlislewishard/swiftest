...
! Executable code
   ...
   associate(npl_old => symba_plA%helio%swiftest%nbody)
      ...
      if (npl_new >= npl_old) then 
         new_symba_plA%helio%swiftest%id(1:npl_old) = symba_plA%helio%swiftest%id(1:npl_old)
      ...
      call move_alloc(new_symba_plA%helio%swiftest%id, symba_plA%helio%swiftest%id )

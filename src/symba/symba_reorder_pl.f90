subroutine symba_reorder_pl(npl, symba_plA)
   ...
   !write(*,*) "************ REORDER ***************"
   do i = 1, npl
      symba_plwkspA%helio%swiftest%name(i) = symba_plA%helio%swiftest%name(sort_index(npl-i+1))
   ...
   call move_alloc(symba_plwkspA%helio%swiftest%name, symba_plA%helio%swiftest%name)

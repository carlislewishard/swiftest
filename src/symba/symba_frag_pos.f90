subroutine symba_frag_pos (mtot, rhill, x, v, m_frag, x_frag, v_frag)
   ...
   do i=1, nfrag ! fragment velocity (scale each by mass and send in each just different direction)
      v_frag(:,i) =  v_frag_norm * ((cos(phase_ang + theta * i)) * v_col_unit_vec(:) + &
                                 (sin(phase_ang + theta * i)) * tri_pro_unit_vec(:))
      x_frag(:,i) =  r_circle    * ((cos(phase_ang + theta * i)) * v_col_unit_vec(:)  + &
                                 (sin(phase_ang + theta * i)) * tri_pro_unit_vec(:))
